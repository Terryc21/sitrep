...
**Context:** Derived from a complete audit → fix cycle on a production iOS app (a few dozen issues found, all fixed in one session)
...
file_timestamps:
  ...
  Sources/Features/Settings/Views/SettingsView.swift: "2026-03-05T09:15:00Z"
  # ... one entry per file referenced in issues[]

issues:
  - id: 1
    finding: ".featureFlagHandler — full handler but zero UI triggers"
    ...
    suggested_fix: "Add trigger to SettingsView and/or the search surface"
...
## Reference: What Informed This Design

This design came out of a full audit → fix cycle on a production iOS app: a few dozen
findings across five layers, all fixed in one session, with the fix phases grouped by
hand.

**What worked:** audit accuracy was high — every finding verified before it was acted on.
The rating system made prioritization fast, and grouping phases by effort (trivial /
small / dead-code) turned out to be the intuitive axis.

**What was manual, and therefore what this handoff design exists to automate:** phase
grouping, pattern discovery for fixes (reading existing dialogs to match local
conventions), per-task prompting, and build verification.
